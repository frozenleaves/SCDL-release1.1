...
def copy_file():
    ...
    for fd in os.listdir(test):
        f = os.path.join(test, fd)
        files = os.listdir(f)
        file_src = [os.path.join(f.replace('train_mcy', 'augment_dic'), x) for x in files]
        src = [s.replace('test/', '') for s in file_src]
...
if __name__ == '__main__':
    ...
    # split_dataset = SplitDataset(dataset_dir='/home/zje/CellClassify/train_dataset/train_data_20x_new2/augment_mcy',
    #                              saved_dataset_dir='/home/zje/CellClassify/train_dataset/train_data_20x_new2/train_mcy',
    #                              show_progress=False)
